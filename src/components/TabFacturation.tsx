...
interface TabFacturationProps {
  ...
  // Grille tarifaire du service (onglet "Actes & Tarifs", toutes catégories)
  // et pharmacie, pour sélectionner directement un acte ou un médicament
...
export default function TabFacturation({
  ...
  // Recherche/sélection dans la grille tarifaire (tous actes, toutes
  // catégories) pour ajout direct à la facture avec le prix officiel.
  const [acteSearchQuery, setActeSearchQuery] = useState("");
  const filteredActes = React.useMemo(() => {
    const q = acteSearchQuery.toLowerCase().trim();
    const sorted = [...actes].sort((a, b) => a.nom.localeCompare(b.nom, "fr"));
    if (!q) return sorted.slice(0, 6);
    return sorted.filter((a) => a.nom.toLowerCase().includes(q)).slice(0, 8);
  }, [actes, acteSearchQuery]);

  const handleAddActeALaFacture = (acte: ActeTarifaire) => {
    const newLine: FactureLigne = {
      id: generateUid(),
      designation: acte.nom,
      qte: 1,
      prix: acte.prix,
      montant: acte.prix
    };
  ...
  // Recherche/sélection dans le stock pharmacie pour ajout direct à la
  // facture avec le prix de vente officiel du médicament.
  const [medSearchQuery, setMedSearchQuery] = useState("");
  const filteredMedicaments = React.useMemo(() => {
    const q = medSearchQuery.toLowerCase().trim();
    const sorted = [...medicaments].sort((a, b) => a.nom.localeCompare(b.nom, "fr"));
    if (!q) return sorted.slice(0, 6);
    return sorted.filter((m) => m.nom.toLowerCase().includes(q)).slice(0, 8);
  }, [medicaments, medSearchQuery]);

  const handleAddMedicamentALaFacture = (med: Medicament) => {
    const newLine: FactureLigne = {
      id: generateUid(),
      designation: `${med.nom}${med.dosage ? " " + med.dosage : ""}`,
      qte: 1,
      prix: med.prixVente,
      montant: med.prixVente
    };
    setFactLignes((prev) => [...prev, newLine]);
  };
            ...
            <div className="space-y-3">
              ...
              <div className="bg-stone-50 p-3 rounded-xl border border-stone-200 space-y-2">
                <span className="text-xs font-semibold uppercase tracking-wider text-stone-600 block">
                  📋 Actes tarifés (grille du service)
                </span>
                <input
                  type="text"
                  placeholder="Rechercher un acte (consultation, pansement, accouchement...)"
                  value={acteSearchQuery}
                  onChange={(e) => setActeSearchQuery(e.target.value)}
                  className="w-full text-xs border border-stone-200 rounded-lg px-3 py-1.5 bg-white"
                />
                {filteredActes.length === 0 ? (
                  <p className="text-2xs text-stone-400 italic px-1">
                    {actes.length === 0 ? "Aucun acte tarifé renseigné. Ajoutez-en dans l'onglet \"Actes & Tarifs\"." : "Aucun acte ne correspond à la recherche."}
                  </p>
                ) : (
                  <div className="space-y-1 max-h-32 overflow-y-auto">
                    {filteredActes.map((a) => (
                      <div key={a.id} className="flex items-center justify-between bg-white rounded-lg px-2.5 py-1.5 text-xs border border-stone-100">
                        <span className="font-semibold text-stone-700 truncate">{a.nom}</span>
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="font-mono font-bold text-stone-600">{a.prix.toLocaleString("fr-FR")} F</span>
                          <button
                            type="button"
                            onClick={() => handleAddActeALaFacture(a)}
                            className="text-primary-600 hover:text-primary-800 font-bold"
                          >
                            + Ajouter
                          </button>
              ...
              <div className="bg-stone-50 p-3 rounded-xl border border-stone-200 space-y-2">
                ...
                {filteredMedicaments.length === 0 ? (
                  <p className="text-2xs text-stone-400 italic px-1">
                    {medicaments.length === 0 ? "Aucun médicament en stock." : "Aucun médicament ne correspond à la recherche."}
                  </p>
                ) : (
                  <div className="space-y-1 max-h-32 overflow-y-auto">
                    {filteredMedicaments.map((m) => (
                      <div key={m.id} className="flex items-center justify-between bg-white rounded-lg px-2.5 py-1.5 text-xs border border-stone-100">
                        <span className="font-semibold text-stone-700 truncate">{m.nom} {m.dosage}</span>
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="font-mono font-bold text-stone-600">{m.prixVente.toLocaleString("fr-FR")} F</span>
                          <button
                            type="button"
                            onClick={() => handleAddMedicamentALaFacture(m)}
                            className="text-primary-600 hover:text-primary-800 font-bold"
                          >
                            + Ajouter
                          </button>
              ...
              <div className="bg-stone-50 p-3 rounded-xl border border-stone-200 space-y-2">
                <span className="text-xs font-semibold uppercase tracking-wider text-stone-600 block">Autre prestation / acte hors liste</span>
                <input
                  type="text"
