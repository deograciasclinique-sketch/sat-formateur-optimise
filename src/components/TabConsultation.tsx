...
export default function TabConsultation({
  ...
  const handlePrescrireExamen = () => {
    ...
    if (onUpdateLaboExamens) {
      ...
      const total = nouveauxExamens.reduce((s, e) => s + (e.prix || 0), 0);
      alert(`Examen(s) prescrit(s) (${noms}) — Total ${total.toLocaleString("fr-FR")} F.\nEnvoyé(s) au laboratoire ET mis à disposition dans l'onglet Facturation (avec leur prix) pour le patient "${consPatient}".`);
  ...
  const handleLinkLabExamenToTemp = (exam: ExamenLabo) => {
    ...
    // On stocke le résultat uniquement dans sa case dédiée (tempLabResults →
    // labResults à l'enregistrement) : on n'écrit plus rien dans le champ
    // "Examen physique / clinique générale", qui doit rester réservé à
    // l'examen clinique du médecin.
    setTempLabResults([...tempLabResults, exam]);
    alert(`Examen de laboratoire (${exam.analyses || exam.examen}) transféré avec succès dans la case "Résultats de laboratoire" de la consultation !`);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-xs space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="text-xs uppercase font-semibold tracking-wider text-stone-500 block mb-1 flex items-center gap-1.5">
            ...
            <div className="pt-3 border-t border-stone-100 space-y-2">
              ...
              {tempLabResults.length > 0 && (
                <div className="pt-2 mt-1 border-t border-primary-100">
                  <span className="text-2xs uppercase font-bold tracking-wider text-primary-700 flex items-center gap-1 mb-1.5">
                    <FlaskConical className="w-3.5 h-3.5" />
                    Case des examens validés par le laboratoire ({tempLabResults.length})
                  </span>
                  <div className="space-y-1.5">
                    {tempLabResults.map((exam) => (
                      <div
                        key={exam.id}
                        className="flex items-start justify-between gap-2 bg-primary-50 border border-primary-200 rounded-lg px-3 py-2"
                      >
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-primary-900">{exam.analyses || exam.examen}</p>
                          <p className="text-2xs text-stone-600 mt-0.5">
                            Résultat : <span className="font-semibold">{exam.resultat || "En attente de saisie"}</span>
                            {exam.interpretation && (
                              <span className={`ml-1.5 font-bold ${
                                exam.interpretation === "Critique" ? "text-danger-700" :
                                exam.interpretation === "Anormal" ? "text-warning-700" : "text-success-700"
                              }`}>({exam.interpretation})</span>
                            )}
                          </p>
                        </div>
                        <button
                          ...
                          className="text-stone-400 hover:text-danger-600 font-semibold shrink-0 cursor-pointer"
                          title="Retirer de cette case"
                        >
                          ×
                        </button>
                      </div>
